...
if (!window.FancyUpload) var FancyUpload = {};

FancyUpload.Attach = new Class({
	...
	initialize: function(list, selects, options) {
		...
		options.fileClass = options.fileClass || FancyUpload.Attach.File;
	...
	onSelectSuccess: function() {
		if (this.fileList.length > 0) {
			...
			this.selects[1].setStyle('display', 'inline-block');
	...
	onFileRemove: function() {
		if (this.fileList.length == 0) {
			this.selects[0].setStyle('display', 'inline-block');
			this.selects[1].setStyle('display', 'none');
...
FancyUpload.Attach.File = new Class({
	...
	render: function()
	{
		...
		var progress = new Element('img', {'class': 'file-progress', src: 'assets/images/bar.gif'}).inject(this.ui.size, 'after');
